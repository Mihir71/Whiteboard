...
exports.renameCanvas = async (req, res) => {
  try {
    ...
    console.log("Rename request:", { canvasId, name, userId });
    ...
    if (canvas.owner.toString() !== userId) {
      console.log("Unauthorized rename attempt:", {
    ...
    console.log("Canvas renamed successfully:", { canvasId, newName: name });
    res.json({ message: "Canvas renamed successfully" });
  } catch (error) {
    console.error("Error renaming canvas:", error);
    res
      .status(500)
      .json({ error: "Failed to rename canvas", details: error.message });
  }
};
